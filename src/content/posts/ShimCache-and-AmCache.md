...
![Application calling into Windows through the IAT](https://miro.medium.com/v2/resize:fit:1336/format:webp/1*wHujq2PCLgnzpWa1jkp77A.png) </center>
...
I know what you’re thinking… Yes, [shims can be abused](https://attack.mitre.org/techniques/T1546/011/) but don’t worry my fellow defenders there’s good news , shims have built-in limitations that keep them from turning into total chaos.

1. **Shims Run in User Mode and Not Kernel Mode**

2. **No Direct Security Bypasses ( But Tricks exist …)**

3.  **App-Specific ( But Tricks exist …)**
...
```sql wrap preserveIndent
process where host.os.type == "windows" and event.type == "start" and 
  process.name : "sdbinst.exe" and
  process.args : "?*" and
...
🔗 [Potential Application Shimming via Sdbinst | Elastic Security [8.18] | Elastic](https://www.elastic.co/guide/en/security/8.18/potential-application-shimming-via-sdbinst.html)

🔗 The Real Shim Shady: [PowerPoint Presentation](https://files.brucon.org/2015/Tomczak_and_Ballenthin_Shims_for_the_Win.pdf)

🔗 [Pillowmint: FIN7’s Monkey Thief](https://www.trustwave.com/en-us/resources/blogs/spiderlabs-blog/pillowmint-fin7s-monkey-thief/)

🔗 [Get2 Downloader & SDBbot RAT Analysis | Proofpoint US](https://www.proofpoint.com/us/threat-insight/post/ta505-distributes-new-sdbbot-remote-access-trojan-get2-downloader)

🔗 [eu-15-Pierce-Defending-Against-Malicious-Application-Compatibility-Shims-wp.pdf](https://www.blackhat.com/docs/eu-15/materials/eu-15-Pierce-Defending-Against-Malicious-Application-Compatibility-Shims-wp.pdf)

🔗 [Secrets of the Application Compatilibity Database (SDB) — Part 1 — Alex Ionescu’s Blog](https://web.archive.org/web/20191115211402/http://www.alex-ionescu.com/?p=39)
...
```cmd showLineNumbers=false frame=none
...
A ShimCache entry ≠ proof of execution. It only confirms the file was present and scanned by the Shim engine.
But don’t worry this is just one piece of the puzzle. We can combine it with AmCache, Prefetch, SRUM, and other logs to build a full timeline.
:::
...
🔗 [Understanding Microsoft’s Application Compatibility Cache (ShimCache) in Digital Forensics](https://www.cyberengage.org/post/forensic-collection-of-execution-evidence-through-appcompatcache-shimcache-amcache-hiv)

🔗 [Let’s Talk About Shimcache — The Most Misunderstood Artifact](https://www.youtube.com/watch?v=7byz1dR_CLg)

🔗 [IR-Guidebook-Final.pdf](https://cdn-dynmedia-1.microsoft.com/is/content/microsoftcorp/microsoft/final/en-us/microsoft-brand/documents/IR-Guidebook-Final.pdf)

### Tools for ShimCache Analysis

Eric Zimmerman’s AppCompatCacheParser : 🔗 [GitHub — EricZimmerman/AppCompatCacheParser: AppCompatCache (shimcache) parser. Supports Windows 7 (x86 and x64), Windows 8.x, and Windows 10](https://github.com/EricZimmerman/AppCompatCacheParser)

Mandiant’s ShimCacheParser.py :🔗 [GitHub — mandiant/ShimCacheParser](https://github.com/mandiant/ShimCacheParser)
...
> 
> **_Implication:_** _You cannot assume the structure based on OS alone. Always verify the patch level and use format-aware tools._

🔗 [Analysis of the AmCache](https://cyber.gouv.fr/sites/default/files/2019/01/anssi-coriin_2019-analysis_amcache.pdf) **Blanche Lagny’s research remains the gold standard for understanding both legacy and modern AmCache formats.**
...
> Only Category 1 involves actual execution and even then, only GUI apps that needed shimming.
...
🔗 [GitHub — EricZimmerman/AmCacheParser](https://github.com/EricZimmerman/AmCacheParser)
...
🔗 [GitHub — log2timeline/regipy](https://github.com/log2timeline/regipy)

:::tip[Pro Tip]

Always use updated parsing tools older versions may misinterpret newer AmCache structures._

Check Task Scheduler for: `\Microsoft\Windows\Application Experience\Microsoft Compatibility Appraiser` If disabled, AmCache may be incomplete.
:::

🔗 Read my full post on **Prefetch** here : [Windows Forensics : Prefetch. Prefetch is a utility that is intended… | by @omayma | Aug, 2025 | Medium](https://medium.com/@omaymaW/windows-forensics-prefetch-8447dbb6cd9b)